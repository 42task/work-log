...
const claudeRoot = path.resolve(moduleDir, "../claude");
...
export const MANAGED_CLAUDE_FILES = [
...
export function buildClaudeHookCommands({ scope, targetRoot }) {
  if (scope === "project") {
    return {
      userPromptSubmit:
        '/usr/bin/python3 "$CLAUDE_PROJECT_DIR/.claude/worklog-hook/cache_user_prompt.py"',
      stop: '/usr/bin/python3 "$CLAUDE_PROJECT_DIR/.claude/worklog-hook/log_turn.py"',
    };
  }
...
function upsertManagedHookEntry(entries, eventName, desiredHook, isManagedPredicate) {
  const nextEntries = Array.isArray(entries)
    ? entries.map((entry) => {
        ...
        const hooks = entry.hooks.map((hook) => {
          if (isManagedPredicate(eventName, hook)) {
            changed = true;
            return desiredHook;
          }
          return hook;
  ...
  const alreadyPresent = nextEntries.some(
    (entry) =>
      ...
      entry.hooks.some((hook) => isManagedPredicate(eventName, hook)),
  );
...
function isManagedClaudeHook(eventName, hook) {
  if (!hook || typeof hook !== "object" || Array.isArray(hook)) {
    return false;
  }
...
export function mergeClaudeSettingsJsonText(existingText, commands) {
  let parsed = {};
...
function resolveClaudeRoot(scope, targetRoot) {
  const resolved = path.resolve(targetRoot);
  if (scope === "global") {
    return resolved;
  }
  return path.join(resolved, ".claude");
}
...
async function writeManagedClaudeFiles(managedRoot) {
  await mkdir(managedRoot, { recursive: true });

  for (const filename of MANAGED_CLAUDE_FILES) {
    await copyFile(
...
export async function installClaudeHook({ scope, targetRoot, force = false }) {
  if (!["global", "project"].includes(scope)) {
    throw new Error(`Unsupported scope: ${scope}`);
  }
  ...
  const resolvedTarget = path.resolve(targetRoot);
  if (scope === "project") {
    const projectStats = await stat(resolvedTarget).catch(() => null);
    if (!projectStats || !projectStats.isDirectory()) {
      throw new Error(`Project path does not exist: ${resolvedTarget}`);
    }
  }

  const claudeConfigRoot = resolveClaudeRoot(scope, resolvedTarget);
  const managedRoot = path.join(claudeConfigRoot, MANAGED_DIRNAME);
  const commands = buildClaudeHookCommands({
    scope,
    targetRoot: claudeConfigRoot,
  });

  await mkdir(claudeConfigRoot, { recursive: true });
  await ensureManagedDirectory(managedRoot, force);
  await writeManagedClaudeFiles(managedRoot);
  await writeManifest(managedRoot, scope);

  const settingsPath = path.join(claudeConfigRoot, "settings.json");
  const existingSettings = await readTextIfExists(settingsPath);
  await writeFile(
    settingsPath,
    mergeClaudeSettingsJsonText(existingSettings, commands),
    "utf8",
  );
...
export function resolveCodexCliTarget({ scope, projectPath }) {
  if (scope === "global") {
...
export function resolveClaudeCliTarget({ scope, projectPath }) {
  if (scope === "global") {
